    ...
    public Collection<Move> calculateLegalMoves(Board board) {
        ...
        for (final int currentOffset : BISHOP_OFFSETS) {
            ...
            while (IsDestinationPositionValid(destinationPosition)) {
                ...
                if (AnyBishopFileExclusions(this.piecePosition, destinationPosition)) {
...
//----------------------------------------------------------------------------------------------------------------------
     ...
     * @return whether the Bishop is on the first file
     */
     ...
     * @param currentPosition where the Bishop is on the board
     * @param currentOffset   the current offset used for calculating the Bishop's destination position
     * @return whether the Bishop is on the eighth file
     */
    ...
    /**
     * A simpler way to determine whether the Bishop is on the first or eighth file.
     * For a Bishop, a move cannot happen more than one file away.
     *
     * @param currentPosition     where the Bishop currently is
     * @param destinationPosition where the Bishop wants to move to
     * @return whether there are any exclusions for the Bishop
     */
    private static boolean AnyBishopFileExclusions(final int currentPosition, final int destinationPosition) {
        ...
        return Math.abs(currentFile - destinationFile) > 1;
    }
}
